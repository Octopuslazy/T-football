...
export default class StartScreen extends PIXI.Container {
    constructor() {
        ...
        try {
            ...
            let tex = null;
            try {
                const assetsGet = PIXI.Assets && PIXI.Assets.get;
                if (assetsGet)
                    tex = PIXI.Assets.get(key) || null;
            }
            catch (e) { }
            if (!tex) {
                try {
    ...
    drawBackground() {
        ...
        if (this.bgSprite && this.bgSprite.texture) {
            try {
                ...
                const tw = (tex.width && tex.width > 0) ? tex.width : ((tex.orig && tex.orig.width) || ((tex.baseTexture && tex.baseTexture.width) || 1));
                const th = (tex.height && tex.height > 0) ? tex.height : ((tex.orig && tex.orig.height) || ((tex.baseTexture && tex.baseTexture.height) || 1));
    ...
    createButton(label, color) {
        ...
        g.endFill();
        const t = new PIXI.Text(label, { fontFamily: 'Roboto', fontSize: 24, fill: 0x000000 });
        t.anchor.set(0.5);
        c.addChild(g);
        c.addChild(t);
    ...
    resize() {
        ...
        try {
            const g = this.playBtn.__bg;
            const t = this.playBtn.__label;
            g.clear();
            ...
            t.x = 0;
            t.y = 0;
            this.playBtn.x = layoutCenterX;
            this.playBtn.y = firstY;
        }
        catch (e) { }
        // Update other button visuals
        try {
            const g2 = this.otherBtn.__bg;
            const t2 = this.otherBtn.__label;
            g2.clear();
            g2.beginFill(0x3498db);
            g2.drawRoundedRect(-btnWidth / 2, -btnHeight / 2, btnWidth, btnHeight, Math.max(6, Math.round(btnHeight * 0.15)));
            g2.endFill();
            t2.style = { ...t2.style, fontSize };
            t2.anchor.set(0.5);
            t2.x = 0;
            t2.y = 0;
            this.otherBtn.x = layoutCenterX;
            this.otherBtn.y = firstY + btnHeight + 18;
        }
        catch (e) { }
        // update hitArea size
        try {
            this.hitArea.width = window.innerWidth;
            this.hitArea.height = window.innerHeight;
        }
        catch (e) { }
    }
}
