...
export default class Ball extends PIXI.Container {
    constructor(gameState, goal) {
        ...
        this._curveControl = null;
        this._curveEnd = null;
        this._moveStartTime = 0;
        this._moveDuration = 800; // ms, will scale with power/distance
    ...
    setupInteraction() {
        ...
        this.on('pointerup', (e) => this.onDragEnd(e));
    ...
    onDragMove(event) {
        if (!this._isDragging || this._isMoving || this._goalScored || this.gameState.gameOver)
            return;
    ...
    drawFlightPath(start, control, end) {
        // Clear existing
        if (this._pathGraphics) {
            try {
