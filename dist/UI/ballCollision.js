...
var CollisioneState;
(function (CollisioneState) {
    CollisioneState["NONE"] = "NONE";
    CollisioneState["POST_IN"] = "POST_IN";
    CollisioneState["POST_OUT"] = "POST_OUT";
    CollisioneState["CROSS_IN"] = "CROSS_IN";
    CollisioneState["CROSS_OUT"] = "CROSS_OUT";
    CollisioneState["KEEPER_SAVED"] = "KEEPER_SAVED";
    CollisioneState["REACH_NET"] = "REACH_NET";
})(CollisioneState || (CollisioneState = {}));
export class BallCollision extends Container {
    ...
    checkCollision(ball, goal) {
        ...
        if (currentScale < 0.4 && currentScale > 0.19) {
            if (this.state === CollisioneState.KEEPER_SAVED)
                return;
            if (this.state === CollisioneState.POST_IN)
                return;
            if (this.state === CollisioneState.CROSS_IN)
                return;
            if (this.state === CollisioneState.REACH_NET)
                return;
            if (this.state === CollisioneState.POST_OUT)
                return;
            ...
            const isFullyInside = bleft >= nleft &&
                bright <= nright &&
                btop >= ntop &&
                bbottom <= nbottom;
            if (isFullyInside) {
    ...
    isCircleRect(cx, cy, radius, rx, ry, rw, rh) {
    ...
    INGoal(goal, ball) {
        if (this.state === CollisioneState.POST_OUT)
            return;
        if (this.state === CollisioneState.CROSS_OUT)
            return;
        if (this.state === CollisioneState.REACH_NET)
            return;
        if (this.state === CollisioneState.KEEPER_SAVED)
