...
export var GoalkeeperAction;
(function (GoalkeeperAction) {
    GoalkeeperAction["Case1"] = "1";
    GoalkeeperAction["Case2"] = "2";
    GoalkeeperAction["Case3"] = "3";
    GoalkeeperAction["Case4"] = "4";
    GoalkeeperAction["Case5"] = "5";
    GoalkeeperAction["Case6"] = "6";
    GoalkeeperAction["Case7"] = "7";
    GoalkeeperAction["Case8"] = "8";
    GoalkeeperAction["Case9"] = "9";
    GoalkeeperAction["Case10"] = "10";
    GoalkeeperAction["Reset"] = "Reset";
...
export default class Goalkeeper extends Container {
    ...
    Jump(jump) {
        if (!this.isGrounded || this.isPrepared)
            return;
    ...
    //#region Case Perform
    PerformFall(action) {
        if (this.isFallen && action !== GoalkeeperAction.Reset)
            return;
        switch (action) {
    ...
    init(spineAsset) {
        ...
        console.log('🔍 Spine structure:', {
            ...
            animations: this.spine.skeleton?.data?.animations?.map((a) => a.name)
        });
        // Force visibility
    ...
    updateScale() {
        if (!this.spine)
            return;
        // Lấy thông tin goal
        if (!this._goal || !this._goal.goalSprite)
            return;
    ...
    //#endregion
    //#region  catch ball
    TimetoCatchBall(targetX, targetY, jumpForce = 0) {
    ...
    calculateMaxY(action) {
        const force = this.getJumpForceForCase(action);
        if (force === 0)
            return this._initialPosition.y;
     ...
     * @param targetY
     */
    checkBestCaseForHeight(targetX, targetY) {
        if (this.isDiving || this.isFallen || this.isPrepared)
            return;
    ...
    setTargetBall(ballGame) {
        this._targetBall = ballGame;
    }
    updateAI() {
        if (!this._targetBall || !this._targetBall.isFlying || this._targetBall.isNetAnim)
            return;
        if (this._hasAIActed || this.isDiving || this.isFallen || this.isPrepared)
            return;
